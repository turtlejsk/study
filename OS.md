...
# 2. 네트워크
  ...
  ## 2.3. 기타질문
    ...
    - Q: Server에서 FIN을 전송하기 전에 전송한 패킷이 Routing 지연이나 패킷 유실로 인한 재전송으로 인해 FIN 패킷보다 늦게 도착한다면 어떻게 될까?
      A: 이런 현상에 대비해 client는 server로부터 FIN을 수신하더라도 일정시간 세션을 남겨 놓고 잉여 패킷을 기다린다.
    - Q: 초기 Seq #을 0부터 시작하지 않고 난수를 생성하는 이유는?
      A: Connection을 맺을 때 사용하는 포트는 유한 범위 내에서 사용하고 시간이 지남에 따라 재사용된다. 따라서 두 통신 호스트가 과거에 사용된 포트 번호 쌍을 사용할 가능성이 존재한다.
  ...
  ### 2.13.2. POST Method
      ...
      - 대용량 데이터 전송에 적합
      - 보안상 안전하다.
  ## 2.14. Cookie와 Session
  - 필요성
    - HTTP 프로토콜은 Connectionless/Stateless와 같은 특성 때문에 모든 요청 간 의존관계가 없다.
    - 현재 접속한 사용자가 이전에 접속했던 사용자와 같은 사용자인지 알 수 있는 방법이 없다.
    - 연결을 유지하지 않기 때문에 리소스 낭비가 줄어드는 장점이 있지만, 통신 마다 새로 연결하기 때문에 클라이언트는 매 요청마다 인증을 해야한다.
    - 사용자를 판별하기 위한 상태를 유지하기 위한 기술이 쿠키와 세션이다.
  
  ### 2.14.1. Cookie
    - 클라이언트 로컬에 저장되는 키와 값이 들어있는 파일
    - 이름, 값, 유효시간, 경로 포함
    - 클라이언트의 상태 정보를 브라우저에 저장
    - 동작방식
      1. 웹브라우저가 서버에 요청
      2. 상태를 유지하고 싶은 값을 쿠키로 생성
      3. 서버가 응답할 때 HTTP 헤더에 쿠키를 포함해서 전송
      4. 전달받은 쿠키는 웹브라우저에서 관리하고 있다가 다음 요청 때 쿠키를 헤더에 넣어서 전송

  ### 2.14.2. Session
    - 일정 시간동안 같은 브라우저로부터 들어오는 요청을 하나의 상태로 보고 그 상태를 유지하는 기술
    - 웹 브라우저를 통해 서버에 접송한 이후부터 브라우저를 종료할 때까지 유지되는 상태
    - 동작방식
      1. 웹브라우저가 서버에 요청
      2. 서버가 해당 웹브라우저에 유일한 Session ID를 부여
      3. 서버가 응답할 때 HTTP 헤더에 Session ID를 포함해서 전송
      4. 웹브라우저는 이후 웹브라우저를 닫기까지 다음 요청 때 부여된 Session ID가 담겨있는 쿠키를 HTTP헤더에 넣어서 전송 

  ### 2.14.3. Cookie와 Session의 차이점
    1. 저장위치
       - Cookie: 클라이언트
       - Session: 서버
    2. 보안
       - Cookie: 취약
       - Session: 쿠키를 이용해 Session ID만 저장하고 이 값으로 구분해서 서버에서 처리하므로 비교적 좋음
    3. Lifecycle
       - Cookie: 만료시간에 따라 브라우저를 종료해도 계속해서 남아있을 수 있다.
